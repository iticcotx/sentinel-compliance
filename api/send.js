// Vercel serverless function — emails the Sentinel compliance digest via Gmail.
// Set env vars in Vercel: GMAIL_USER, GMAIL_APP_PASSWORD  (and optional MAIL_TO).
...
module.exports = async (req, res) => {
  ...
  let scopes = []; try { scopes = (JSON.parse(raw || "{}").scopes) || []; } catch(e){}
  const allowed = scopes.length ? scopes : ["provider","facility","other"];

  const today = new Date(); today.setHours(0,0,0,0);
  const rows = [];
  for (const it of (data.items||[])) {
    if (it.active === false) continue;
    if (!allowed.includes(it.scope)) continue;
    if (!it.expires) continue;
    const e = new Date(it.expires); if (isNaN(e)) continue;
    const days = Math.floor((e - today) / 86400000);
    if (days > 90) continue;
    const status = days < 0 ? "EXPIRED" : days <= 30 ? "CRITICAL" : "DUE SOON";
    rows.push({ cat: it.category, entity: it.entity, exp: e, days, status });
  }
  ...
  const trs = rows.map(r => {
    ...
    const when = r.days < 0 ? Math.abs(r.days)+"d ago" : "in "+r.days+"d";
  ...
  const html = `<div style='font-family:Segoe UI,Arial,sans-serif;max-width:760px;margin:auto;color:#0f172a'>
    <div style='background:linear-gradient(135deg,#14b8a6,#0f766e);color:#fff;padding:22px 26px;border-radius:14px 14px 0 0'>
      <div style='font-size:13px;letter-spacing:2px;opacity:.85'>SENTINEL &middot; COMPLIANCE COMMAND CENTER</div>
      <div style='font-size:22px;font-weight:800;margin-top:4px'>[${allowed.join("/")}] digest — ${stamp}</div></div>
    <div style='border:1px solid #e6ebf1;border-top:none;border-radius:0 0 14px 14px;padding:22px 26px'>
      ...
      <p style='color:#94a3b8;font-size:12px;margin-top:22px'>Sent by Sentinel (cloud).</p></div></div>`;
  ...
  try {
    ...
    await t.sendMail({ from:user, to, subject:`Sentinel [${allowed.join("/")}] digest - ${nE} expired, ${nC} critical, ${nD} due`, html });
