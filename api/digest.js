// Scheduled daily digest — Vercel Cron calls this (GET) once a day; it emails a
// summary of everything expiring within 90 days to MAIL_TO (default GMAIL_USER).
// Also works on demand if you open /api/digest in a browser.
...
module.exports = async (req, res) => {
  ...
  const html = `<div style='font-family:Segoe UI,Arial,sans-serif;max-width:760px;margin:auto;color:#0f172a'>
    <div style='background:linear-gradient(135deg,#14b8a6,#0f766e);color:#fff;padding:22px 26px;border-radius:14px 14px 0 0'>
      <div style='font-size:13px;letter-spacing:2px;opacity:.85'>SENTINEL · DAILY COMPLIANCE DIGEST</div>
    ...
    <div style='border:1px solid #e6ebf1;border-top:none;border-radius:0 0 14px 14px;padding:22px 26px'>
      ...
      <p style='color:#94a3b8;font-size:12px;margin-top:22px'>Automatic daily digest from Sentinel.</p></div></div>`;
  ...
  try {
    ...
    await t.sendMail({ from: user, to, subject: `Sentinel daily digest — ${nE} expired, ${nC} critical, ${nD} due`, html });
